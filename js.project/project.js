...
bascket.addEventListener('click', () => {
    window.location.href = './karzina/index.html'
})
...
const url = 'https://65d6c4e3f6967ba8e3be85e9.mockapi.io/f' // countries api
fetch(url)
    .then(response => response.json()) // accessing the API data as JSON

    .then(data => {
        data.forEach((product) => {
            container.appendChild(createCardContainer(product))
        })
    })
...
btns.forEach((btn, index) => {
    btn.addEventListener('click', () => {
        circle.forEach((item, i) => {
            if (i === index) {
                item.classList.add('circle-sec-3-active')
            } else {
                item.classList.remove('circle-sec-3-active')
            }
        })
...
// for (let i of shtuk2) {
//     div.appendChild.createSection(i)
// }
